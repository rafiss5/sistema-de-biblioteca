...
public class ObraController {
     ...
     * @param obra Obra com dados atualizados
     * @throws IllegalArgumentException Se a obra for inválida ou não existir
     */
    public void atualizarObra(Obra obra) throws IllegalArgumentException {
        validarObra(obra);

        if (!obraExiste(obra.getCodigo())) {
            throw new IllegalArgumentException("Obra não encontrada para atualização. Código: " + obra.getCodigo());
        }

        if (!obraDAO.salvar(obra)) {
            throw new IllegalStateException("Falha ao atualizar obra no banco de dados");
        }
    }
